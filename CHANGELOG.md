...
把建模全流程的「侦探决策回路」从中央协议改为分布式补丁，按渐进式加载边界落入各角色工作流文件——每个角色只记住自己的肌肉反应，不读全局协议。不再有中央接线表；修改一处不牵动他处，测试回归成本随耦合归零而下降。

- **删除中央协议强制引用**：`references/侦探协议.md` 降级为人类可读章程，退出执行链路，仅保留「分布位置索引」（纯信息性）。全库确认无执行引用。
