...
- 根入口从全量生产协议收缩为轻量路由：保留建模手、编程手、论文手和严格门禁，但普通思路任务不再默认创建账本、表格或加载全部角色。
- 新增 `目标语义卡.md`：算法与知识库加载前，先锁定评价主体、瞬时成功、跨对象/跨域聚合、输出合同和区分反例，专门拦截把附件列误当目标函数、求和/并集/交集混淆等题意错误。
- `M1` 改为两层审查：先做目标语义否决，失败即停止；通过后才按真实风险检查自由度、边界、文献、算法和格式，避免次要清单淹没核心错误。
- 算法资产、创新协议、竞争账本和论文规范改为条件触发；禁止遍历整个 `assets/` 或 `knowledge/`。文献检索明确位于独立初读之后、模型冻结之前，写作阶段只做题录核验。
- 风险项、算法对照和决策空间表不再设普通问题的固定数量或空表要求；完整竞赛交付仍保留 `M1/P1/P2/W1/W2` 与现有审计脚本。

## 2.4.0 - 2026-08-22

### 判据语义、边界条件与优化器漏洞防护

- 新增 `判据语义与边界审计.md`：在搜索前锁定判据主体、对象范围、量词和可执行测试，并为动态状态登记有效域、边界后行为及来源。
- 代表点/平均/松弛代理只能用于候选搜索；最终 incumbent 必须用题意支持的严格合同重新计分。无来源的 `clip/max/min`、饱和、落地停止或越界延拓使候选进入 `UNCERTIFIED`。
- 竞争性账本升级为 schema v2，每问新增 `model_contract_audit`。`challenge_audit.py` 现在机械拦截对象/量词未定义、边界来源缺失、代理差值未解决或 incumbent 未经严格合同认证的账本。
- `M1/P1/P2/W1/W2` 同步接入判据与边界合同；创新候选若优势仅来自宽松代理或未证边界行为，不得进入 `VERIFIED/ADOPTED`。

## 2.3.0 - 2026-08-18

### 逐题双轨完成与竞争性挑战

- 新增 `references/竞争型问题协议.md`：阶段 0 逐题声明 `conformance/competitive`。真实性总闸继续裁决正确性；竞赛型增加内部标尺、结构性挑战、incumbent 棘轮、预算和停止证书，禁止把首个可行解当成完成。
- 新增条件性交付物 `results/竞争性搜索账本.json` 与 `challenge_audit.py`。审计器核对硬约束、软目标、baseline、内部标尺、incumbent、至少一个非参数微调的结构性挑战、预算和五类合法停止证书。
- `M1` 增加完成模式、目标合同和挑战预算核对；`P2` 增加挑战审计；`W1/W2` 只有在 `PROVEN_OPTIMAL` 证书下允许使用“最优”，其他状态统一写“当前最好已知方案”并披露 gap 或未知前沿。
- 温度策略改为双轨求解：达标型严格收敛；竞赛型冻结可行主链，在隔离分支扩张目标、表示、分解、模型信息或求解结构，只有真实性通过且更优的挑战者才能晋升 incumbent。
- 创新审计新增高竞争价值候选淘汰守卫：预算内的 `competitive_relevance: high` 候选不能靠主观理由直接 `DROPPED`；须提供原型或硬约束证据，预算不足则保留为 `HYPOTHESIS` 并登记未探索前沿。
- 新增竞争性搜索与高价值候选回归测试，保持“不强制创新数量”，同时堵住“允许空创新清单 → 竞赛型达标即停”的漏洞。

## 2.2.0 - 2026-08-18

### 模型来源自洽三钩（本地 P9/P10/P11 · 加钩子不动总闸）

- **本地 P9 · 模型—文献一致性（挂 M1）**：关键模型与参数文献锚定（无值时显式声明「由数据反演，无独立依据」）、声明适用波段与文献一致性、失效窗预先承诺并附候选窗对照表。守卫：候选窗禁止按结果反选（等同事后补出口），偏差出现在未预判窗 → 标注「未预判触发」并走分歧报告；多窗不一致报告可辨识带。落点：建模设计理论 + 质检清单 3 条 + M1 核对。
- **本地 P10 · 自由参数伦理（挂 P2）**：无文献依据的自由参数须过四问（物理意义 / 净残差贡献超噪声底 2σ / 能否被更底层物理假设解释 / 是否与既有参数退化），任一不过参数不得进模型。`parameter_rationale` 四字段入 `创新证据清单.json`，`innovation_audit.py` 同步强制校验。守卫：2σ 为默认可覆盖阈值；总闸优先——结构矛盾存在时「加参数未过 2σ」不得成为跳过高阶解释的依据。落点：建模设计理论 + 创新证据协议 + 审计脚本。
- **本地 P11 · 模型来源自洽（挂 W2）**：自审框架新增 `M0-2`，核心是「为什么选这个模型」（来源论证）；奥卡姆比较仅当无结构矛盾时适用；窗口交叉核验报告可辨识带；参数文献对照。落点：自审框架 + W2 核对。
- 顶层：强制执行协议加第10条「模型—文献—数据三方交叉核验」+ 完成判定一条。全部为加钩子，2.1.0 已有体系（决策总闸、四重锁定、可辨识带、分歧报告幕）原样保留。

## 2.1.0 - 2026-08-18

### 决策优先级总闸（物理优先 · 价值仲裁层）

- **新增总闸（SKILL.md 顶部）**：物理判据（能量守恒、量纲、口径一致性、预期范围等）是外部约束，不是算法输出。本总闸是价值仲裁层，优先级高于所有执行协议；触发时 `温度策略.md` 第26条「禁止中途引入新模型思想」自动失效——是「被覆盖」，不是「例外」。
- **两级设计**：第一级前置预判——`题目分析报告.md` 第10节写成含 [触发条件/应对方案/失败出口] 三列的决策树，写不出失败出口 → 阻断（不进入求解）；第二级输出形态切换——运行时遇到结构矛盾自动切换「分歧报告模式」，不设「停机等用户」阻断点，不等用户。
- **两种完成形态，均为论文完成状态**：「解决」幕（点估计+完备证据）与「分歧报告」幕（区间+各方证据+承认无法辨识）。
- **四项硬规则**：收敛档压制解除（物理矛盾触发时模型升级是必须执行的，非「算出来的」）；事后补出口禁止（已执行即兴路径不得补写出口洗白，强制转分歧报告幕或重新求解）；阈值不写死（统一写入第10节预判摘要）；分歧报告幕完成状态明确（完成门禁=区间完整、证据列明、差异量化、无物理依据声明，不要求解决幕）。
- **分布落点**：建模手 `工作流程.md` 预判摘要硬性结构 + `M1` 出口完备性核对；论文手 开始门禁预判摘要核验、完成门禁分歧报告幕完成状态；创新证据协议 判据通过后强制路径切换 + 辨识性护栏；自审框架 `M0-1` 外部合理性门禁；章节模板 P6 三选二 + 总闸切换；题面拆解 阶段0 自洽性与合理性检查。

## 2.0.0 - 2026-08-17

### 决策回路架构重构：分布式神经节

把建模全流程的「决策回路」从中央协议改为分布式补丁，按渐进式加载边界落入各角色工作流文件——每个角色只记住自己的肌肉反应，不读全局协议。不再有中央接线表；修改一处不牵动他处，测试回归成本随耦合归零而下降。

- **建模手 `工作流程.md` · 异常值处理协议（本地 P0+P1）**：数据异常先过第一段 3σ 统计显著性门控（偏离 < 3σ 局部噪声底 → 统计不显著，剔除记录，结束流程），通过才进第二段物理归因分叉（高阶机制签名 → 禁止剔除/滑窗，必须触发模型升级）。门控挂 `M1`。
- **编程手 `验证完备性.md` · 第 7 类触发式验证「合成数据回收检验」（本地 P4）**：两种独立口径对同一量给出 d1、d2 且 |d1−d2| > 2·√(SEM1²+SEM2²) 时强制触发三明治回收；|Δd| < 报告不确定度/3 判无偏；必须输出 Δd 数值区间。含诚实护栏：`synthetic_recovery.py` 是模板，必须替换为题目真实模型，直接跑内置演示就宣称通过视为伪造验证。
- **论文手 `自审框架.md` · M0 概念可复核性（置顶最高优先级门禁）**：每个关键数值三句话讲清「附件原始行 → 该值」完整路径，说不清视为拷贝，禁止入文。对全文生效。
- **论文手 `章节模板.md` · 章节叙事默认弧（本地 P6，数字门控）**：核心问题求解章节按「冲突 → 探究 → 解决」三段展开，每幕 ≥1 真实且可复核的数值，无数值支撑的幕自动折叠不许单独成节。
- **论文手 `自审框架.md` · 不确定度与模型局限写法（本地 P5+P8）**：系统偏差禁止强行平均，必须报告可辨识带 [d_min, d_max] 并说明主导机制（缺则未过自审）；升级验证失败必须输出固定句式「未解之谜」段落（缺则一票否决）。
- **创新专项 `创新证据协议.md` · 机制主张四重锁定（本地 P7）**：矛盾 / 判据 / 复现 / 来源四锁缺一，机制主张在论文中自动降级为「推测」并加注「需进一步验证」；审计员卡 `创新证据清单.json`。
- **建模手 `建模设计理论.md` · 退化自洽句（本地 P3）**：模型族扩展版必须含 `当 [参数] → [极限] 时式(X)退化为式(Y)`，缺句未过建模自审。
- **新增 `synthetic_recovery.py` 工具箱**：合成数据三明治回收演示（正向生成 → 加噪 → 反向回收 → 报 Δd）。修复混叠阶误选、disp 网格量化残留和噪声劈谷三问题；30–60 dB 稳健。默认 SNR=50 dB（保证特征高出噪声底 ≥3σ）、重复 100 次（系统偏差均值估计可信），脚本为名下数值的单一事实源。

## 1.4.0 - 2026-08-16

### 创新系统底层重构

- 将创新从固定数量、固定算法和固定展示位置改为“探索环自由发散 + 证据环严格收敛”，允许最终没有核心创新。
- 新增 `HYPOTHESIS/PROTOTYPED/VERIFIED/ADOPTED/DROPPED` 状态链，论文只能把 `ADOPTED` 项称为创新。
- 新增创新发现工作流、物理几何简化证明、求解创新路由和创新证据协议；历史题目的贪心、可达域和管理启示降级为案例，不再作为跨题铁律。
- 物理几何简化按 G1 精确等价、G2 保守逼近、G3 可控近似、G4 经验降阶分类，要求保持性质、尺度、证明或误差界、反例和失效条件。
- 删除全局三类异构求解器、每创新配图、六处展示、固定双参数灵敏度和参考答案驱动调参等错误激励；验证改为按真实结论风险触发。
- 新增 `innovation_audit.py` 与 `results/创新证据清单.json`，检查真实代码入口、证明、结果、复现命令、公平基线和失效边界，并接入 P2/W1/W2。
- 新增创新专项回归测试；完整测试套件共 133 项通过。

## 1.3.0 - 2026-08-16

### 视觉论证与非数据图

- 新增 `references/视觉论证与配图策略.md`，把配图生成单位从固定图型/数量改为读者疑问、核心主张、证据载体和章节落点构成的论证缺口。
- 新增 `tools/diagram/SKILL.md`：连续二维/三维几何优先使用 GeoGebra Classic；流程、架构和拓扑使用 NetworkX 建结构、Matplotlib 绘制；数据图继续由 `tools/figure/` 处理。
- 无法自动渲染 GeoGebra 时输出 `.geogebra.txt` 逐行指令与生成说明，并在图表论证清单中标记 `PENDING_RENDER`；该状态允许阶段性交付但阻断最终 `P2/W2`。
- 正式数据图统一使用 Python/Matplotlib，MATLAB 保留为可选求解语言但不再作为正式配图后端。
- 编程手新增 `results/图表论证清单.json`，逐项记录读者疑问、证据来源、视觉角色、图前引入、章节落点、图后观察/推论和 `KEEP/MERGE/APPENDIX/DROP` 决策。
- `figure_audit.py` 从三类图数量门禁改为图文件、论证清单、待渲染状态和逐题正式证据覆盖审计；`raw/process/result` 仅保留为可选来源标签和旧项目兼容检查。
- Word/LaTeX 不再使用统一八幅图最低值；LaTeX 以 `fig:qN-*`、`tab:qN-*` 或 `eq:qN-*` 检查逐题正式证据覆盖。
