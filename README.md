...
- 🎯 **轻量题意冻结**：先用目标语义卡区分评价主体、跨对象/跨时间聚合和输出合同，再按需加载知识库，避免模板列或早期假设锁死错误目标函数。
- 🏁 **双轨完成**：逐题区分达标型与竞赛型；前者正确可复现即收敛，后者建立内部标尺、挑战预算、incumbent 和停止证书，防止首个可行解过早封盘。
- 💻 **双语言实现**：支持 Python 和 MATLAB，按选中的模型与功能动态检查依赖。
- 📊 **完整结果输出**：生成结果表格、原始数据图、模型运行过程图和最终结果图。
- 🎨 **视觉论证驱动配图**：先识别论证缺口再选择文字、公式、表格、数据图或非数据图；数据图、流程、架构和拓扑使用 Python/Matplotlib，连续几何优先 GeoGebra，统一执行源文件、SVG/高分辨率 PNG 与成图自检闭环。
...
- 🤝 **可选 Subagent 协作**：用户可按需启用官方规则核验（仅 2026 当届）、附件盘点、文献与模型调研、算法原型、独立实验、双语言对照或术语核验；默认全部关闭。
...
| ② | [编程手](references/roles/编程手/SKILL.md) | 编写并运行 Python/MATLAB 求解代码，生成结果和候选证据；竞赛型执行预算化挑战 | `P1` 最小可运行结果、`P2` 编程终检 | 代码、结果表格、候选图、创新清单、条件触发的竞争性搜索账本、图表与复现清单 |
| ③ | [论文手](references/roles/论文手/SKILL.md) | 基于真实结果构建论证并生成论文 | `W1` 证据大纲、`W2` 论文终检 | 每题至少一个正式证据单元；默认 Word，用户指定 LaTeX/PDF 或双格式时按指定范围交付 |
...
只实现现有模型，使用 MATLAB 求解并导出结果，再用 Python 生成正式数据图。
根据现有代码结果生成完整论文.docx。
...
│   ├── mechanism_q1_* / geometry_q1_*          # 机理与几何
│   ├── flow_q1_* / architecture_q1_*           # 流程与架构
│   ├── topology_q1_*                            # 网络拓扑
│   ├── *.geogebra.txt / *_生成说明.md            # 无法自动渲染时的 GeoGebra 产物
│   └── _qa/                       # 自动生成的灰度质检预览
...
| [数模非数据图](tools/diagram/SKILL.md) | GeoGebra 几何机理图、NetworkX + Matplotlib 流程/架构/拓扑图、指令降级与放置契约 |
...
默认只生成 Word；用户指定 Word、LaTeX/PDF 或两者时按指定范围生成。当届官方提交要求仍决定实际可提交的版本。
...
篇幅质量目标由 `W1` 根据当届官方页数与实际图表密度制定，不设跨竞赛通用的最低字数或页数。校验器按正文与附录边界核对官方上限，并检查逐题证据覆盖、题注与正文引用、参考文献双向对应和 PDF 完整性。
...
- 正式数据图、流程图、架构图和拓扑图统一使用 Python/Matplotlib；连续几何优先使用 GeoGebra Classic，MATLAB 仅作为可选求解语言。
- 多面板图先声明主面板和辅助证据；信息量或证据权重不同时使用非对称布局，不机械等分成仪表盘。
- 面板内只保留短标题，优先直接标注或共享图外图例；限制密集逐点标记、装饰性纹理和冗余色条。
- 每张候选图先登记读者疑问、核心主张、证据来源、章节落点和采用决策；`raw/process/result` 只是可选来源标签，不按类别、题号或图型凑数。
...
- 导出后自动生成灰度预览，并审计图表论证清单、逐题正式证据覆盖、待 GeoGebra 渲染状态、格式配对、PNG DPI、物理尺寸和 SVG 文本；Agent 必须在论文预计尺寸下复述第一视觉结论，检查遮挡、尺度、颜色区分和跨面板一致性，发现问题后回到源代码重绘。
...
工作流针对 CUMCM（全国大学生数学建模竞赛）设计，其他竞赛须按当届官方要求另行配置页面、摘要、编号、页数和提交格式。
...
`2.2.0` 新增模型来源自洽三钩（本地 P9 模型—文献一致性、P10 自由参数伦理、P11 模型来源自洽），全部加钩不动总闸：模型选型、参数取值与结果判定三方交叉核验，2σ 等默认可覆盖阈值，`parameter_rationale` 入 `创新证据清单.json` 并由审计脚本强制校验。`2.1.0` 曾新增决策优先级总闸（物理优先·价值仲裁层）：`题目分析报告.md` 第10节预判摘要成为硬性结构，物理判据触发时收敛档自动失效、自动切换「分歧报告」幕，两种完成形态（解决幕/分歧报告幕）均为论文完成状态。`2.0.0` 曾把建模决策回路重构为分布式补丁，新增 3σ 异常门控、合成数据回收检验、可辨识带、未解之谜固定句式与机制主张四重锁定。详细内容见 [CHANGELOG.md](CHANGELOG.md)。
