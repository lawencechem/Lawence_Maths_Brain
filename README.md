...
`2.1.0` 新增决策优先级总闸（物理优先·价值仲裁层）：`题目分析报告.md` 第10节预判摘要成为硬性结构，物理判据触发时收敛档自动失效、自动切换「分歧报告」幕，两种完成形态（解决幕/分歧报告幕）均为论文完成状态。`2.0.0` 曾把建模决策回路重构为分布式补丁，新增 3σ 异常门控、合成数据回收检验、可辨识带、未解之谜固定句式与机制主张四重锁定。详细内容见 [CHANGELOG.md](CHANGELOG.md)。
