...
## 算法大全扩展（司守奎《算法大全》第 01/02/03/04/23 章）

本扩展补充整数规划求解细节、动态规划建模框架、约束优化经典方法与现代启发式参数经验。全文细节见 `11-算法大全检索索引.md`。

### A. 分支定界与割平面（整数规划求解）

分支定界对 LP 松弛求解，按最优值分数变量分支（产生两个子问题），用上/下界剪枝；割平面（Gomory 割）逐步切去非整数可行域。竞赛中直接调 `intlinprog`/LINGO `@GIN`，但论文需说明"分支定界思想"。0-1 规划常用处理：指派、选址、背包。

```matlab
[x, fval] = intlinprog(f, 1:n, A, b, Aeq, beq, lb, ub);   % 1:n 表示全部整数变量
```

### B. 动态规划逆推框架（多阶段决策）

阶段 $k$ 状态 $s_k$、决策 $u_k$、状态转移 $s_{k+1}=T(s_k,u_k)$，最优指标递推

$$
f_k(s_k)=\min_{u_k}\left\{ v_k(s_k,u_k)+f_{k+1}(s_{k+1}) \right\},\quad f_{N+1}(s_{N+1})=0
$$

关键：明确阶段、状态（无后效性）、决策、转移与指标；从末端逆推。背包、生产-库存、资源分配均可用此框架统一表达。

### C. 约束优化经典方法

- **拉格朗日乘子法**：$L(x,\lambda)=f(x)-\sum_i\lambda_i g_i(x)$，最优解满足 KKT 条件（对不等式约束为 $\lambda_i\ge0,\ \lambda_i g_i(x)=0$）。
- **罚函数法**：$F(x)=f(x)+M\sum_i\max(0,g_i(x))^2$，用递增罚因子 $M$ 逼近原问题；可作初值/可行性处理，最终精解用 `fmincon`。

### D. 现代启发式参数经验（第 23 章）

| 算法 | 关键参数 | 常用经验 |
|------|---------|---------|
| 模拟退火 | 初温 $T_0$、退火系数、终止温度 | 接受准则 $\exp(-\Delta/T)>\mathrm{rand}$；$T_0$ 使初始接受率≈0.8 |
| 遗传算法 | 种群规模、交叉率 $p_c$、变异率 $p_m$ | $p_c\approx0.6\text{–}0.9$，$p_m\approx0.001\text{–}0.1$；锦标赛选择常用 |
| 蚁群 | 信息素蒸发系数 $\rho$、α/β | $\rho\approx0.5$，α/β 平衡启发式与信息素 |

### E. 交叉引用

对偶/灵敏度、下料切割见 `09-运筹学专题算法说明.md`；多目标/鲁棒见本文 14/15 节；算法大全第 26/27 章（经济金融、生产运作优化）经薄索引按需精读。
