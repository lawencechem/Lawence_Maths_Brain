...
## 算法大全扩展（司守奎《算法大全》第 09/24/28 章）

补充时间序列建模完整流程与灰色 GM(1,1) 检验细节。全文见 `11-算法大全检索索引.md`。

### A. 时间序列建模流程（第 24 章）

确定性时间序列常用：移动平均、指数平滑、差分指数平滑、自适应滤波、趋势外推。随机型时间序列（ARMA 族）标准流程：

1. **平稳性检验**：ADF 检验；不平稳则差分 $d$ 次（ARIMA(p,d,q)）。
2. **定阶**：看 ACF（q 截尾 → MA(q)）与 PACF（p 截尾 → AR(p)），或 AIC/BIC 比较。
3. **参数估计**：极大似然/最小二乘。
4. **残差诊断**：Ljung-Box 白噪声检验；残差仍相关则增大阶数。
5. **预测与回测**：滚动预测，报告误差。

```python
from statsmodels.tsa.arima.model import ARIMA
fit = ARIMA(y, order=(p, d, q)).fit()
print(fit.summary())               # 含 AIC/BIC、残差检验
forecast = fit.forecast(steps=h)
```

### B. 灰色 GM(1,1) 完整步骤与检验（第 28 章）

1. **级比检验**：$\lambda(k)=x^{(0)}(k-1)/x^{(0)}(k)$ 应落在 $(e^{-2/(n+1)},e^{2/(n+1)})$；否则需变换。
2. **累加生成**：$x^{(1)}(k)=\sum_{i=1}^{k}x^{(0)}(i)$。
3. **建立灰微分方程**：$x^{(0)}(k)+a\,z^{(1)}(k)=b$，其中 $z^{(1)}(k)=\tfrac12(x^{(1)}(k)+x^{(1)}(k-1))$（紧邻均值）。
4. **最小二乘参数**：$(a,b)^T=(B^TB)^{-1}B^TY$。
5. **白化方程**：$\dfrac{dx^{(1)}}{dt}+a x^{(1)}=b$，预测

$$
\hat{x}^{(1)}(k+1)=\left(x^{(0)}(1)-\frac{b}{a}\right)e^{-ak}+\frac{b}{a},\quad
\hat{x}^{(0)}(k+1)=\hat{x}^{(1)}(k+1)-\hat{x}^{(1)}(k)
$$

6. **后验差检验**：残差方差比 $C=S_2/S_1$ 与小误差概率 $p$；$C<0.35$ 且 $p>0.95$ 为优，$C<0.5$ 且 $p>0.8$ 为合格。$C$、$p$ 是论文必须报告的两项。

```python
def gm11(x0):
    n = len(x0); x1 = np.cumsum(x0)
    z1 = 0.5*(x1[1:] + x1[:-1])
    B = np.c_[ -z1, np.ones(n-1) ]; Y = x0[1:]
    a, b = np.linalg.lstsq(B, Y, rcond=None)[0]
    pred = lambda k: (x0[0]-b/a)*np.exp(-a*(k-1)) + b/a   # x1(k)
    x1_hat = np.array([pred(k) for k in range(1, n+2)])
...
- 插值要求曲线过已知点（拉格朗日、分段/样条、二维 griddata），用于填充与可视化。
- 拟合不要求过点，最小二乘求参数（`polyfit`/`lsqcurvefit`/`curve_fit`），用 $R^2$ 与残差评价。
- 竞赛中"插值保形、拟合平滑"：先判数据误差来源再决定用哪个。

### D. 交叉引用

小样本预测选 GM(1,1) 并与 ARIMA 对比（见本文 1/6 节）；多变量时间序列可用 VAR/LSTM（本文 10 节）；模型族与经典案例见 `08-`、`10-` 资产。
