...
## 算法大全扩展（司守奎《算法大全》第 08/22 章）

补充 AHP 权重计算的工程方法、一致性检验细节与模糊综合评价的矩阵运算。全文见 `11-算法大全检索索引.md`。

### A. AHP 权重与一致性检验细节（第 08 章）

判断矩阵 $A=(a_{ij})$（$a_{ij}>0$，$a_{ij}=1/a_{ji}$）。常用近似求权：

- **和积法**：每列归一化 → 行求和 → 再归一化得权重 $w$。
- **方根法**：每行连乘开 $n$ 次方 → 归一化。

一致性检验：$\lambda_{\max}\approx\sum_i\dfrac{(Aw)_i}{n w_i}$，

$$
\mathrm{CI}=\frac{\lambda_{\max}-n}{n-1},\qquad \mathrm{CR}=\frac{\mathrm{CI}}{\mathrm{RI}}
$$

$\mathrm{RI}$ 取值（$n=1..9$）：$0,0,0.58,0.90,1.12,1.24,1.32,1.41,1.45$。$\mathrm{CR}<0.1$ 通过；否则重新征询两两比较。层次总排序：自顶向下加权合成，最后同样做一致性检验。

```python
def ahp_weight(A):
    n = A.shape[0]
    w = (A / A.sum(axis=0)).sum(axis=1) / n          # 和积法
    lam = (A @ w / w).mean()
    CI = (lam - n) / (n - 1)
    RI = {3:0.58,4:0.90,5:1.12,6:1.24,7:1.32,8:1.41,9:1.45}.get(n, 0)
    return w, CI/RI if RI else 0
```

### B. 模糊综合评价矩阵运算（第 22 章）

1. 确定评语集 $V=\{v_1,\dots,v_m\}$ 与指标集 $U=\{u_1,\dots,u_n\}$。
2. 由隶属函数或统计构造模糊关系矩阵 $R=(r_{ij})_{n\times m}$（指标 $i$ 对评语 $j$ 的隶属度）。
3. 权重 $A=(a_1,\dots,a_n)$（AHP/熵权给出）。
4. 综合评判 $B=A\circ R$：
   - 主因素突出型：$b_j=\max_i\min\{a_i,r_{ij}\}$；
   - 加权平均型：$b_j=\sum_i a_i r_{ij}$（更细腻，推荐）。
5. 按最大隶属度原则给出评语；可再归一化 $B$。
...
- **模糊聚类**：模糊相似矩阵 → 求传递闭包（平方自合成直至 $t(R)\circ t(R)=t(R)$）→ 取截集 $\lambda$ 得聚类，随 $\lambda$ 得动态聚类图。
- **模糊识别**：最大隶属度原则、贴近度（格贴近度等）用于模式分类。

### D. 交叉引用

熵权法、TOPSIS、DEA 见本文 3/4/9 节；模糊综合在 `08-经典建模方法说明.md` 第 10 节亦有建模视角；算法大全第 29 章（多元分析：聚类/判别/PCA）经薄索引按需精读。
