...
## 算法大全扩展（司守奎《算法大全》第 10/11/12/29/30 章）

补充方差分析、回归检验细节、判别分析与偏最小二乘回归。全文见 `11-算法大全检索索引.md`。

### A. 方差分析（第 11 章）

**单因素方差分析**：总离差平方和分解 $SST=SSA+SSE$（组间+组内），

$$
F=\frac{SSA/(k-1)}{SSE/(n-k)}\sim F(k-1,n-k)
$$

$F>F_\alpha$ 拒绝"各组均值相同"。**双因素**：进一步分解主效应与交互项。**正交试验**：用正交表安排多因素试验并做方差分析，减少试验次数。

```python
from scipy import stats
F, p = stats.f_oneway(*[g for g in groups])   # 单因素
```

### B. 回归检验细节（第 12 章）

多元回归 $y=X\beta+\varepsilon$：$R^2$、调整 $R^2$、整体 $F$ 检验、单个系数 $t$ 检验与置信区间。**逐步回归**（前进/后退/双向）用 $F$ 检验决定变量进出（`stepwisefit`/`stepwise`）。非线性回归用 `nlinfit`/`lsqcurvefit`，初值重要。

```matlab
[b, se, pval, inmodel, stats] = stepwisefit(X, y);   % inmodel=选中变量
```
...
- **距离判别**：算到各类（重心）的 Mahalanobis 距离，就近归类。
- **贝叶斯判别**：结合先验概率与误判损失，按后验概率最大归类。
- **Fisher 判别**：投影使类间/类内方差比最大，降维后再判别。
...
### D. 偏最小二乘回归 PLS（第 30 章）

适用于自变量高度相关、样本少、多因变量的场景：同时提取 $X$、$Y$ 的潜在成分（最大化协方差），再做回归。比普通多元回归/逐步回归更稳健。

```python
from sklearn.cross_decomposition import PLSRegression
pls = PLSRegression(n_components=k).fit(X, Y)
```

### E. 交叉引用

预处理/聚类/PCA/因子/典型相关见本文 1~6 节；算法大全第 29 章含聚类与主成分的完整案例，第 30 章含 PLS 案例，经薄索引按需精读；回归类预测联动 `02-预测类算法说明.md`。
