...
## 算法大全扩展（司守奎《算法大全》第 06/15/18/20 章）

补充排队论生灭过程、常微分方程数值解法、变分法与偏微分方程数值解。全文见 `11-算法大全检索索引.md`。

### A. 排队论：生灭过程与 M/M/c（第 06 章）

到达为 Poisson（生率 $\lambda$）、服务为指数（灭率 $\mu$）时，系统人数是**生灭过程**，稳态概率满足平衡方程，得

$$
M/M/1:\ P_0=1-\rho,\quad L=\frac{\rho}{1-\rho},\quad L_q=\frac{\rho^2}{1-\rho},\quad W=\frac{1}{\mu-\lambda}
$$

M/M/c（$c$ 个服务台）系统空闲概率

$$
P_0=\left[\sum_{k=0}^{c-1}\frac{(c\rho)^k}{k!}+\frac{(c\rho)^c}{c!(1-\rho)}\right]^{-1},\quad
L_q=\frac{(c\rho)^c\rho}{c!(1-\rho)^2}P_0
$$

$\rho=\lambda/(c\mu)$。排队指标可用于"加服务台 vs 减少等待"的成本-效益决策。

### B. 常微分方程数值解法（第 15 章）

对初值问题 $y'=f(x,y)$，$y(a)=y_0$：

- **Euler 显式**：$y_{k+1}=y_k+h f(x_k,y_k)$（一阶精度）。
- **改进 Euler（梯形预测-校正）**：$y_{k+1}=y_k+\frac h2[f(x_k,y_k)+f(x_{k+1},y_k+hf(x_k,y_k))]$（二阶精度）。
- **四阶 Runge-Kutta（RK4）**：竞赛默认选择，`ode45` 即自适应 RK 法。**刚性系统**用 `ode15s`。

```python
from scipy.integrate import solve_ivp
sol = solve_ivp(f, [a, b], [y0], method='RK45', rtol=1e-8)
```

### C. 变分法与动态优化（第 18 章）★

变分法求泛函极值。对泛函 $J[y]=\int_{x_1}^{x_2}F(x,y,y')dx$，极值曲线满足 **Euler 方程**

$$
\frac{\partial F}{\partial y}-\frac{d}{dx}\frac{\partial F}{\partial y'}=0
$$

端点固定即自然边界条件；最短路径、最速降线、最小旋转面均由此导出。**动态优化**：求控制函数 $u(t)$ 使 $\int L(x,u)dt$ 最小，用最优控制/最大值原理（Pontryagin）。适用于轨道、调度、生长类问题。

### D. 偏微分方程数值解（第 20 章）★

三类定解边界：Dirichlet（给值）、Neumann（给导数）、混合。差分格式：

- **椭圆型**（Poisson $\Delta u=f$）：五点差分格式离散 Laplacian，解大型线性方程组（迭代/直接法）。
- **抛物型**（热传导 $u_t=u_{xx}$）：显式格式稳定条件 $r=\Delta t/h^2\le\frac12$；隐式（Crank-Nicolson）无条件稳定。
- **双曲型**（波动 $u_{tt}=c^2u_{xx}$）：中心差分，满足 CFL 条件。

```python
# 热传导显式
r = dt/h**2; assert r <= 0.5
U[1:-1] = U[1:-1] + r*(U[2:]-2*U[1:-1]+U[:-2])*dt   # 五点式简化
```

### E. 交叉引用

排队论基础见本文第 2 节；微分方程建模与稳定性见 `08-经典建模方法说明.md` 第 3/4 节；ODE 数值解法与 PDE 原章含 MATLAB 程序，经薄索引按需精读（算法大全第 15/20 章）。
