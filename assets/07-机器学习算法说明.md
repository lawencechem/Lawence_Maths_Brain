...
## 算法大全扩展（司守奎《算法大全》第 19 章）

补充 BP 多层前馈网络原理与训练细节。全文见 `11-算法大全检索索引.md`。

### A. BP 神经网络（第 19 章）

**结构**：输入层 → 隐含层 → 输出层，神经元输出经激活函数（常用 sigmoid $\sigma(x)=1/(1+e^{-x})$）。

**前向**：$z^{(l)}=W^{(l)}a^{(l-1)}+b^{(l)}$，$a^{(l)}=\sigma(z^{(l)})$。

**反向传播（误差）**：定义损失（如平方误差 $E=\tfrac12\sum(t_j-y_j)^2$），输出层误差 $\delta_j^{(out)}=y_j(1-y_j)(t_j-y_j)$，隐含层 $\delta_i^{(h)}=a_i(1-a_i)\sum_j w_{ij}\delta_j^{(out)}$，权重更新

$$
w \leftarrow w+\eta\,\delta\,a^{(l-1)}
$$

$\eta$ 为学习率。迭代至误差收敛。

**训练要点**：数据归一化（避免饱和）；学习率过大震荡、过小过慢；加动量项加速与防局部极小；**训练/验证/测试划分**防止过拟合；早停（验证误差上升即停）。
...
```matlab
net = feedforwardnet(10);  net = train(net, X', T');  % 经典 BP
```

### B. 适用与局限

- **适用**：分类/回归、非线性映射、小规模特征；竞赛中常与特征工程（05-统计）结合。
- **局限**：黑箱、需调参、局部极小、小样本易过拟合——论文需报告学习率/层数/训练-验证误差曲线。
- **替代**：数据量大时优先 XGBoost/LSTM（见 `02-预测类算法说明.md`）；需要可解释性时用随机森林。

### C. 交叉引用

集成学习与异常检测见本文 1~3 节；神经网络预测见 `02-预测类` 第 4/9 节；算法大全第 19 章含蠓虫分类完整案例（含 MATLAB 程序），经薄索引按需精读。
