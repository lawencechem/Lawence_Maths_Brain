...
exportVisualizationData(result, fullfile(projectRoot, "results"));
end
```
...
MATLAB 仅负责求解和导出可追溯结果，不作为正式论文绘图后端：

- 把绘图需要的序列、矩阵、节点边表、坐标、参数和统计量写入 `PROJECT_ROOT/results/` 下的 CSV、MAT 或必要 JSON。
- 同时记录字段、单位、维度和来源步骤，确保 Python 绘图脚本不重新推测语义。
- 数据图使用 `../../../../tools/figure/SKILL.md`；机理、几何、流程、架构和拓扑图使用 `../../../../tools/diagram/SKILL.md`。
- 历史 MATLAB 绘图脚本可以用于结果核对，但不得作为新的正式图或绕过 Python/GeoGebra 配图门禁。
