...
模板来源：从目标竞赛官方网站检索 **2026 年**当届官方模板与规范（只允许 2026 当届，禁止检索其他届次、年份与评阅要点）；用户已明确启用"官方规则核验 Subagent"时，可让其与输入盘点并行取得官方 URL、适用届次（2026）、硬约束、模板路径与哈希。无论是否启用可选 Subagent，论文构建都必须等待模板与规则确认。
...
1. 用户提供的当届官方模板和规则（最高优先）。
2. 从目标竞赛官方网站取得的 **2026 年**当届官方模板和规则（只检索 2026 当届，禁止检索其他届次、年份与评阅要点）。
3. Word 分支以内置 `references/论文模板.docx`（按 2026 年 CUMCM 规范编写）与 `../../../tools/docx/scripts/paper_format.py` 为默认构建基线；LaTeX 分支以 `../../../tools/latex/assets/templates/` 为基线。
4. 规则仅涉及格式、模板、篇幅、提交要求等硬性规定，**不含评阅要点**；按本 Skill 全局盲做规则，禁止检索官方评阅要点/参考答案/赛后解析/赛后同题论文。

生成前记录竞赛名称、届次、语言和所用模板来源（官网 URL 或用户提供）。官方结构、页型、页边距、摘要页、页数、编号和提交格式以 2026 年规范为准。
