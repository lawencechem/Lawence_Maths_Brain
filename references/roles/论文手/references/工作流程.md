...
同时记录两类篇幅约束：一类是当届官方页数、摘要和文件限制，必须满足；另一类是论文完整度质量目标，可以由用户覆盖。**页数上限是硬目标，字数由页数反推**——带图、表格、公式的建模论文正文每页约 500–600 字词单位，官方 20 页 ≈ 10,000–11,000 字词单位；不设独立于页数的全局字数目标（“15000 字”只作无图表纯文本的完整度上限示例，约需 25–30 页）。以 2026 年官方规范为例，正文不超过 30 页；2025 修订稿为“正文尽量 20 页”。要更厚只能加图表格（换证据密度）或用户明确接受超页，禁止“扩字→压排版”跷跷板。

## 四、W1 证据大纲

W1 阶段先产出**页数预算表**并纳入大纲：按官方正文上限逐章分配 目标页数 × 字数 × 图表格数，求和 ≤ 官方上限（字数 = 页数 × 密度 500–600 字词单位/页）；摘要单独预算 ≤ 1 页（含题目与关键词约 850–950 字）。写作中持续用该预算核对篇幅，不等到文档渲染后才第一次统计；结构冻结后才进一次渲染做最终确认。
...
本节命令为**最终冻结**时的交付门禁；论文修改/迭代期间不重复执行（见论文手 `SKILL.md` 的「论文修改/再稿模式」），用户确认最终版本后一次性运行。

Word：

```powershell
python ../../innovation-special/scripts/innovation_audit.py "<PROJECT_ROOT>/results/创新证据清单.json" --project-root "<PROJECT_ROOT>" --strict
...
存在竞赛型子问题时，在 Word/LaTeX 构建前追加：

```powershell
python ../../innovation-special/scripts/challenge_audit.py "<PROJECT_ROOT>/results/竞争性搜索账本.json" --project-root "<PROJECT_ROOT>" --questions <竞赛型题号...> --strict
```
...
python ../../../../tools/latex/scripts/latex_paper.py validate "<PROJECT_ROOT>/完整论文-LaTeX/main.tex" --pdf "<PROJECT_ROOT>/完整论文.pdf" --quality-checks --questions q1 q2 q3 --min-image-dpi 300 --max-pages <当届官方正文上限> --body-start-page <正文起始页> --appendix-start-page <附录起始页>
```
