...
写作前先运行创新审计；存在竞赛型子问题时再按 `../../../竞争型问题协议.md` 运行挑战审计。读取创新清单、竞争性搜索账本（如有）和图表论证清单，在内存或临时工作区建立主张—证据映射：每个结论对应公式、表格、图、代码输出或文献。只有 `ADOPTED` 项可称为创新；`VERIFIED` 只说明证据成立，不自动获得创新地位。竞赛型还要把 baseline、内部标尺、incumbent 和停止证书映射到结果与局限章节。
