...
写作前先运行创新审计，再读取 `results/创新证据清单.json` 和 `results/图表论证清单.json`，在内存或临时工作区建立主张—证据映射：每个结论对应公式、表格、图、代码输出或文献。只有 `ADOPTED` 项可称为创新；`VERIFIED` 只说明证据成立，不自动获得创新地位。映射只用于检查，不作为额外交付物。`raw/process/result` 只是候选证据来源，不构成逐题配额。
