...
论文内容结构与格式遵循 2026 年 CUMCM 官方规范。模板优先级：用户提供的当届官方模板 > 从官方网站检索的 2026 年当届官方模板与规范（`http://www.mcm.edu.cn/`，只检索 2026 当届）> 内置 `references/论文模板.docx`（按 2026 年规范编写）。所用来源与届次必须记录；不检索 2026 以外的届次、年份或评阅要点。

### CUMCM 篇幅的硬约束与质量目标

- 每次任务重新核验当届官方摘要、正文、附录和文件限制；往届数值只能作为历史记录，不能直接沿用。
- **页数上限是硬目标，不设独立全局字数目标**。`W1` 按当届规则、模板和实际图表密度制定逐章页数/内容预算；经验密度只能用于初始估算，不能冒充官方要求或跨题固定门槛。
...
该检查会报告篇幅、公式/图/表数量、图表编号与正文引用、参考文献双向对应以及页数问题。最低篇幅/证据目标取自 `W1`，官方上限取自当届规则，均通过参数显式传入；工具不提供跨竞赛通用默认值。OOXML 校验通过后仍需渲染抽检，因为 XML 合法不等于版面正确。
