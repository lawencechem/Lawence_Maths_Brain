...
该检查会报告篇幅、公式/图/表数量、图表编号与正文引用、参考文献双向对应以及页数问题。默认 15000 字词单位、20 页、公式 5 个和表 3 个是 CUMCM 完整度质量目标；图数由主张—证据映射决定，不设统一最低值。每个子问题的证据覆盖和配图放置由 `results/图表论证清单.json`、`W1` 与 `W2` 检查。2026 年 CUMCM 正文 30 页上限属于已经核验的官方硬约束。使用其他届次或竞赛时必须传入相应阈值；降低默认目标必须记录官方条款或用户要求。OOXML 校验通过后仍需渲染抽检，因为 XML 合法不等于版面正确。
