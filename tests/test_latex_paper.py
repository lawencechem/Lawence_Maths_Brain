...
""",
            ...
            self.assertEqual(report["threshold_overrides"][0]["name"], "require_pdf")

    def test_requires_each_declared_question_to_have_a_formal_evidence_unit(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
...
""",
            ...
            self.assertTrue(report["metrics"]["question_evidence_coverage"]["q1"])
            self.assertFalse(report["metrics"]["question_evidence_coverage"]["q2"])
