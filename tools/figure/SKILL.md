...
description: >-
  从数据剖析到出版级数据图的 Python 单轨可视化工具。先做数据剖析（列类型/样本量/分布/异常值/分组结构/相关性），
  再结合视觉论证目标推荐图型，主动拦截科研画图经典错误，产出 Nature / Science / IEEE / Elsevier / PNAS /
  中文核心期刊级别的成图。覆盖数据 EDA、图表契约、选图决策、出版级绘制、程序自检 + AI 读图闭环、多格式
  导出和文件审计；机理、几何、流程、架构和拓扑图由 tools/diagram/SKILL.md 处理。
...
> 从数据剖析到出版级数据图 | Python + Matplotlib | 自检 + AI 读图闭环
...
本工具融合图表契约与数据剖析、选图决策、自检闭环，并按 `../../references/视觉论证与配图策略.md` 服务数学建模论文证据链。**永远先思考再画**：
...
**只覆盖数据图**：折线、柱状、散点、箱线/小提琴、热力图、误差棒、分布图、相关性矩阵、雷达图、
数据曲面和多面板组合。机理、几何、流程、架构和拓扑图立即路由到 `../diagram/SKILL.md`，不得把非数据关系硬画成统计图。
...
在选图之前，先填写视觉论证契约（详见 `tools/figure/static/core/contract.md`）：

1. **读者疑问**：没有该图时读者无法判断什么
2. **核心结论**：一句话写出这张图必须捍卫的论点
3. **载体比较**：为什么图比文字、公式或表格更直接
4. **证据链**：每个面板对应论点的哪个证据环节
5. **章节落点**：图前引入、插入位置、图后观察与推论
6. **导出规范**：最终尺寸、可编辑文字、源数据、统计口径、导出格式
...
- **语义优先**：图型由数据和论证任务决定；每个面板回答唯一问题，不为制造表面多样性重复展示数据
...
setup_style(journal='general', lang='zh')            # 中文宋体/Times 混排 + 国赛主题
```

`SciencePlots` 装了自动用，没装回退到内置预设。

主题固定为 `'cumcm'`（国赛审美主题：离散用 Okabe-Ito 8 色、连续热力图用 viridis/magma 等
感知均匀色图 + 手动刻度 + 中文宋体/Times 混排，定义见 `scripts/cumcm_theme.py`），
`setup_style()` 不再有 `theme` 参数。数据图与示意图（`../diagram/SKILL.md`）统一用同一
色板，避免论文里两套色系。
...
数学建模场景：同时输出 SVG 与至少 300 DPI PNG；LaTeX 安全链需要时额外导出 PDF。
...
# 2. 数学建模 P2 门禁：图表论证清单 + 子问题证据覆盖（终检时由质检 Subagent 执行）
python "<SKILL_ROOT>/references/roles/编程手/scripts/figure_audit.py" "<PROJECT_ROOT>/figures" --manifest "<PROJECT_ROOT>/results/图表论证清单.json" --questions q1 q2 ... qN --strict
```

- `check_figure.py`：检查单张图的格式合规性（DPI、字体、尺寸、矢量格式），编程手每次导出后自检用。
- `figure_audit.py`：检查图文件与论证清单是否一致、每个子问题是否具有正式证据、拟入文图是否填写放置和图后推论，P2 编程终检时由独立质检 Subagent 执行。
...
| rainbow / jet 色图 | 感知不均匀 | 按数据语义手工构造连续或发散色图，并检查灰度可辨性 |
...
3. **配色固定国赛主题**——离散 Okabe-Ito、连续 viridis/magma + 冗余编码 + 灰度预览
...
| 机理、几何、流程、架构或拓扑图 | `tools/diagram/SKILL.md` |
