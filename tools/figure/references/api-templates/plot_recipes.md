...
候选数据图可以来自以下三类证据，但不要求每个子问题或每类都生成：

| 类别 | 目的 | 推荐图表类型 | 产出阶段 | 采用条件 |
|------|------|-------------|---------|-----------|
| **① 原始数据可视化** | 理解影响建模选择的数据特征 | 直方图、箱线图、密度图、热力图、散点图矩阵、折线图 | 数据剖析后 | 图能改变预处理、假设或模型选择 |
| **② 中间产物可视化** | 展示求解过程，验证模型行为 | 收敛曲线、灵敏度折线、残差图、QQ图、学习曲线 | 模型运行过程中 | 图能验证收敛、误差、边界或算法稳定性 |
| **③ 最终结果可视化** | 支撑核心结论和方法对比 | 对比图、预测折线+CI、气泡图、组合图、地图热力图 | 求解完成后 | 图比结果表更清楚地支持核心判断 |

**全局要求**：每张图先通过视觉论证门禁并登记到 `results/图表论证清单.json`；图型数量和种类不是验收目标。
