...
- [ ] **图型语义**：每个面板回答唯一问题，图型由证据任务决定，不为制造多样性重复数据（反冗余清单见 §11）
...
`tools/figure/scripts/cumcm_theme.py` 是国奖评阅审美的参数化主题（离散用 Okabe-Ito 8 色 +
品牌语义四色 + TINTS 浅底 + 连续色图放行 viridis/magma 等感知均匀色图、只禁 jet/rainbow 等
有害色图 + 手动刻度上限 x≤8/y≤6 + 合规检查）。数据图用 `setup_style()`（主题固定为 `cumcm`），
示意图用 `tools/diagram/SKILL.md` 的共享色板。以下五个坑是实测踩出来的，照抄会翻车：

### 硬坑 1：mpl 3.11 不跨 `font.serif` 列表做字形回退

`font.serif=["Times New Roman","SimSun","SimHei"]` + `font.family="serif"` 会把中文全部渲染成
豆腐块（实测 5 条 missing-glyph 告警）。必须把**具体字体列表直接放进 `font.family`**：

```python
plt.rcParams["font.family"] = ["Times New Roman", "SimSun", "SimHei"]  # ✓ 西文 Times，中文兜底宋体
# plt.rcParams["font.family"] = "serif"  # ✗ CJK 豆腐块
```

### 硬坑 2：`axis('off')` 的判据在 mpl 3.11 变了

`ax.axis("off")` 置 `ax.axison=False`，但**不**翻转 `xaxis.get_visible()` / `_frameon`。
合规检查跳过刻度上限时必须用：

```python
axis_off = not getattr(ax, "axison", True) or not (ax.xaxis.get_visible() and ax.yaxis.get_visible())
```

### 硬坑 3：流程框/拓扑节点的浅底必须手动写死并进白名单

节点填充用的"主色浅底 / 辅色浅底 / 强调色深描边"不是品牌四色本身，`validate_theme_compliance`
会拦。必须从 `TINTS` 取写死 hex，并把 TINTS 并入合规白名单（cumcm_theme 已处理）。

### 硬坑 4：合规检查必须在 `plt.close(fig)` 之前跑

先关图再校验会得到空 `fig.axes` → 静默 0 issues 假通过。顺序必须是：
**savefig → validate_theme_compliance / audit_layout → plt.close(fig)**。

### 硬坑 5：mpl 3.11 的 `get_gridlines()` 网格关闭时也返回对象

`ax.get_ygridlines()` 只要轴带刻度就返回 gridline 对象（默认色 `#b0b0b0`），与 `grid` 是否开启
无关——按「存在性」判断会把普通轴误判成网格，报出幽灵 WARN。必须过滤可见性：

```python
visible = [gl for gl in (*ax.get_ygridlines(), *ax.get_xgridlines()) if gl.get_visible()]
# grid 关 → 空；ax.grid(axis='y') → 非空
```

原型阶段的「0 issues」曾同时被硬坑 4 的静默假通过和这条掩盖，两条都要防。
