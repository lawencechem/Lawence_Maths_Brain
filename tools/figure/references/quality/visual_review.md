...
- **程序能查的有限**：缺字、越界、刻度相交、文字块互相重叠这类**确定性**问题程序能抓（`visual_qa.py`），但"图例正好压在一簇数据点上""标注文字压在曲线上""配色看着发灰分不开"这类**感知性**问题，只有把图当成图像看才发现。
- 本 skill 运行在具备**多模态读图能力**的 Agent 之上——使用当前环境提供的图像查看工具直接读 PNG（例如 Claude Code 的 `Read` 或 Codex 的 `view_image`）。如果当前环境没有图像查看能力，必须明确标记视觉层为 `BLOCKED`，不能把程序自检冒充 AI 读图终检。

## 分工：程序自检 vs AI 读图

| 层 | 工具 | 负责抓 |
|---|---|---|
| 程序自检 | `scripts/visual_qa.py :: audit_layout(fig)` | 缺字乱码、文字越界裁切、刻度标签重叠、**文字块互相重叠**（确定性；文字-文字） |
| Agent 读图 | 本文件清单 + 当前环境图像查看工具读 PNG | 图例压数据、**文字压曲线/数据点/线条**、子图标签对齐、配色/灰度可分、整体观感（感知性） |
...
3. **文字遮盖 / 重叠**
   - **文字-文字重叠**已由程序自检抓过（`audit_layout` 的 WARN），这里复核程序报出的位置是否已修好、有没有漏网。
   - **图例有没有压住数据**（点、线、柱）？
   - **标注文字有没有压在曲线、线段或数据点上面**（文字-线/文字-点程序不查，必须读图确认）？
   - x 轴刻度标签有没有挤成一团、互相穿插？
...
| 标注文字压曲线/数据点 | `annotate` + `xytext` 偏移 + `arrowprops` 锚到空白区；或减少标注数量 |
...
| 配色不可区分/灰度糊 | 换 Okabe-Ito（`cumcm_theme.OKABE_ITO`）+ 加线型/marker |
