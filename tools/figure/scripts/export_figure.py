...
def export_figure(
    ...
    Args:
        ...
        tight: 是否走 bbox_inches='tight'（会改变合同尺寸，论文正式图默认 False）。
    ...
    for fmt in formats:
        ...
        path = f"{basename}.{fmt}"
        _ensure_parent(path)
        # 硬坑：cumcm 主题 RC 设 savefig.bbox="tight"，mpl 3.11 中 bbox_inches=None 会经
    ...
    if grayscale_preview:
        gray_path = _grayscale_from(fig, basename, dpi=dpi, tight=tight)
        if gray_path:
            saved.append(gray_path)
    return saved


def _grayscale_from(fig, basename: str, dpi: int, tight: bool = False) -> str | None:
    """
    导出灰度预览版用于色盲安全检查。
