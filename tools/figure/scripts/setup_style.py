...
def configure_chinese_fonts(serif_for_zh: bool = False) -> str:
    ...
    # 中文期刊常要求中文 + Times New Roman 混排。可靠实现：把含 CJK 的字体置于
    # font.family 列表首位（中文字体自带拉丁字形），不依赖 mpl 的列表字形回退。
    # 经验教训（2026-08）：mpl 3.11.1 在同一进程连续渲染多张图时，对
    # font.family/font.serif 列表的逐字形回退不可靠——西文（Times）在前时，
    # 中文会被替换成豆腐块并报 "Font 'default' does not have a glyph"；
    # 把 CJK 字体放首位则稳定 0 缺失。见 cumcm_theme.py 顶部"硬坑 1"。
    plt.rcParams["font.family"] = [chosen, "Times New Roman", "Times", "DejaVu Sans"]
    if serif_for_zh:
