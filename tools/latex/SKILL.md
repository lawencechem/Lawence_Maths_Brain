...
2. 从目标竞赛官方网站取得的 **2026 年**当届官方 LaTeX 模板项目（只检索 2026 当届，禁止其他届次、年份与评阅要点）。
3. `assets/templates/cumcm/` 构建基线（默认兜底）。

用户提供件优先，官网取得的 2026 模板次之，内置基线最后兜底。复制官方模板目录时保留其 `.cls`、`.sty`、`.bst`、字体和图片资源，不重写导言区或文档类。
...
- `fig:q1-*`、`tab:q1-*` 或 `eq:q1-*` 等正式证据标签是否覆盖 `--questions` 声明的全部子问题；
