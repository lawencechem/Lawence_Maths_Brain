...
def inspect_paper(
    ...
    *,
    ...
    # 2026 官方 LaTeX 模板的摘要用居中标题（{\bfseries 摘\quad 要}），无 abstract 环境；
    # 两种写法都视为已含摘要，避免官方模板误报"缺少摘要环境"。
    has_abstract = bool(re.search(r"\\begin\s*\{abstract\}", source)) or bool(
        re.search(r"摘\s*(?:\\quad\s*)?要", source)
    )
    if not has_abstract:
        issues.append("缺少摘要环境（\begin{abstract} 或含“摘要”的标题至少其一）")
    if not re.search(r"\\(?:keywords|keyword)\s*\{", source, re.I):
        issues.append("缺少关键词命令")

    # \\guide{...} 是 2026 官方 LaTeX 模板的【写作提示】宏调用，模板头要求提交前全部删除；
    # \\placeholder{...} 是同一模板的"填写正文"占位宏。两者都只匹配调用（\guide{ / \placeholder{），
    # 不匹配宏定义（\newcommand{\guide} / \newcommand{\placeholder}），避免误报残留的宏定义。
    placeholders = sorted(set(re.findall(
        r"LATEX_TEMPLATE_[A-Z_]+|\bTODO\b|待补充|请填写|\\guide\{|\\placeholder\{",
        source,
        re.I,
    )))
    if placeholders:
        readable = {
            "\\guide{": "写作提示(\\guide{...})",
            "\\placeholder{": "填写占位(\\placeholder{...})",
        }
        shown = "、".join(readable.get(p, p) for p in placeholders[:8])
        issues.append("仍含模板占位符：" + shown)
    ...
    for question in question_ids:
        matched = any(re.match(
            rf"(?:fig|tab|eq):{re.escape(question)}(?:[-_:]|$)", label, re.I
        ) for label in all_labels)
        question_coverage[question] = matched
        if not matched:
            issues.append(
                f"子问题 {question} 缺少正式证据单元"
                f"（label 应以 fig:{question}-、tab:{question}- 或 eq:{question}- 开头）"
            )
    ...
    metrics = {
        ...
        "question_evidence_coverage": question_coverage,
    }
    labels = {
        "content_units": "字词单位",
        "equations": "公式",
