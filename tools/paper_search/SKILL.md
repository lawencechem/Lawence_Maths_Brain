...
1. **双引擎命中**：核 DOI 与题录。
2. **未命中但有原文线索**：WebSearch 定位期刊官网/百度学术 → 提取题录 → OpenAlex 核 DOI（如《红外与激光工程》前缀 `10.3788/IRLA...`）；能核才引用。
3. **仅知网收录**：主 Agent 出检索式（主题词+期刊+年份+来源类别=核心期刊），用户导出 GB/T 7714 题录（题名/作者前3/刊名/年卷期页/DOI），主 Agent 核验整合。知网登录为浏览器会话/校园网绑定，不可由 Agent 直接驱动。
4. **无法核验**：标 `BLOCKED`，禁编造。
